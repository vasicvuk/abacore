...
/// <summary>
/// Provides an interpreter-based implementation of <see cref="IPolicyCompiler"/>.
/// Directly evaluates the policy AST at runtime without any code generation or compilation.
/// Provides fastest compilation (no compilation needed) but slower runtime performance.
/// Perfect for AOT scenarios and when policies change frequently.
/// </summary>
public sealed class InterpreterCompiler : IPolicyCompiler
{
    ...
    public Runtime.CompiledPolicy CompilePolicy(Policy policy, bool enableCaching = true)
    {
        ...
        Decision evaluationDelegate(EvaluationContext context)
        {
            try
            {
                return policy.Target != null && !EvaluateTarget(policy.Target, context)
                    ? Decision.NotApplicable()
                    : EvaluatePolicy(policy, algorithm, context);
            }
            catch (Exception ex)
            {
    ...
    public Runtime.CompiledPolicy CompilePolicySet(PolicySet policySet, PolicyRepository repository, bool enableCaching = true)
    {
        ...
        foreach (PolicySetElement element in policySet.Elements)
        {
            ...
            {
                // Get referenced policy from repository
        ...
        // Create the evaluation delegate
        Decision evaluationDelegate(EvaluationContext context)
        {
            try
            {
                // Evaluate policyset target if present
                if (policySet.Target != null && !EvaluateTarget(policySet.Target, context))
                {
                    return Decision.NotApplicable();
                }

                // Evaluate child policies using combining algorithm
                Decision decision = algorithm switch
                {
                    CombiningAlgorithm.DenyOverrides => PolicySetEvaluator.EvaluateDenyOverrides(childPolicies, context, policySet),
                    CombiningAlgorithm.PermitOverrides => PolicySetEvaluator.EvaluatePermitOverrides(childPolicies, context, policySet),
                    CombiningAlgorithm.FirstApplicable => PolicySetEvaluator.EvaluateFirstApplicable(childPolicies, context, policySet),
                    CombiningAlgorithm.OnlyOne => PolicySetEvaluator.EvaluateOnlyOne(childPolicies, context, policySet),
                    CombiningAlgorithm.DenyUnlessPermit => PolicySetEvaluator.EvaluateDenyUnlessPermit(childPolicies, context, policySet),
                    CombiningAlgorithm.PermitUnlessDeny => PolicySetEvaluator.EvaluatePermitUnlessDeny(childPolicies, context, policySet),
                    _ => PolicySetEvaluator.EvaluateDenyOverrides(childPolicies, context, policySet)
                };
        ...
        Runtime.CompiledPolicy compiledPolicySet = new()
        {
            ...
            EvaluationDelegate = evaluationDelegate,
            OriginalPolicy = null, // PolicySets don't have an original Policy
            GeneratedCode = $"// PolicySet: {policySet.Id} with {childPolicies.Count} child policies"
        };
    ...
    public void ClearCache() => _compiledPolicyCache.Clear();

    public bool RemoveFromCache(string policyId) => _compiledPolicyCache.TryRemove(policyId, out _);
